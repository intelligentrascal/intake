...
**Feedback (v1.1.0):** Settings → Feedback opens a prefilled GitHub Issue in the browser (Option D — no PAT, no server). Menu bar **Send Feedback…** and About **Send Feedback…** open the same pane. Diagnostics default off; screenshots copy to the pasteboard for ⌘V on GitHub. Optional email is included in the public GitHub issue; leaving it blank prevents that.


Users discover Intake from the **Dock** (on by default) and the **menu bar** (on by default, dense 16pt soft-catch template icon). Dock click, reopen, and a normal launch bring **Settings** forward (last pane or General). Activity is an audit window opened with **Open Activity** from the menu bar, General, or Settings → Activity. First-run shows a one-time tip: Intake lives in the Dock and the menu bar. Show in Dock / Show in menu bar can each be turned off, but **not both** — there must always be an icon that reopens Intake. With Show in Dock off, Intake still appears in the Dock and ⌘Tab while Settings or Activity is open, and returns to menu-bar-only once they close.

**Automatic organizing** is the Settings control for the live watcher (on by default). **Rename when download finishes** (on by default) is independent: it locally normalizes the file name in the watch-folder root as soon as the download is stable. Turning off Automatic organizing disables filing but keeps rename on; turning off Rename disables both. The menu bar still uses Watching / Paused with Pause / Resume. Delete is never silent. AI is never on by default.

**Watch folders (v1.4):** Settings → General lists every watch folder with its status — Watching (organizing on), Paused (per-folder pause toggled), Organizing off (Automatic organizing disabled for that folder), or Needs access — and a per-folder menu: Show in Finder, Change Folder…, Pause Organizing / Resume Organizing, Remove…. Pausing is a temporary halt — like Automatic organizing off, it stops filing while leaving the independent rename-on-stable step to keep running — without changing the folder's Automatic organizing setting. Each status reading carries a `.help` tooltip spelling out what it means. **Add Folder…** offers the system screenshot location (the `com.apple.screencapture` `location` setting when present, otherwise Desktop) as a one-click start, and **Choose Folder…** for anything else; access is always granted through the folder picker (sandbox). Each folder has its own **Automatic organizing**, **Rename when download finishes** and **Wait before organizing**, shown with that folder's row (inline with one folder; in a per-folder disclosure, together with its display name, with several). A folder is rejected if it is the same as, inside, or contains another watch folder, or is one of Intake's category folders (including a date subfolder in one). Soft cap of **5** folders. The last folder can't be removed. Pausing, removing or losing access to one folder leaves the others running; a folder whose access was lost shows **Intake can't see this folder** with **Grant Access…**. General's **Startup and appearance** section has **Open at login**, Show in Dock and Show in menu bar, separate from folder organizing settings.
...
Users can edit filing rules: extensions, destination folder name, enable, order (drag with ⌥⌘↑/↓, or context menu), add/delete custom rules. Deletion and reset-to-default confirm. Built-in categories keep a stable id; renaming a folder name applies to **new** routes only (existing folders are not mass-renamed). Conflicting extensions resolve by list order (first enabled match wins). Empty folders are never pre-created.
...
### Organize existing
Manual one-shot from the menu bar (**Organize Existing…**) or Settings → General → **Existing files** (subtitled so a first-timer knows it previews renames and moves for loose items already in the root). With several watch folders it's a menu: **All Watch Folders** or one folder; each folder is scanned and previewed with its own scoped rules, and the preview names the folder on each group. Scans **watch-folder root only** (does not recurse into Intake-managed category folders). Applies the same ignore policy and rename → route → Activity pipeline as live ingest, and **does not wait** for Wait before organizing. Allowed when Automatic organizing is off (does not turn watching back on). Never runs automatically on launch. Lazy folders only.
...
### Activity
Chronological **audit trail** of rename / move / skip / error (plus cleanup delete and empty-folder removal). It is **not** the Dock default. It is not the Cleanup queue. Rows persist across launches. Double-click a row, or use Reveal in Finder (context menu everywhere; also a toolbar button in the standalone Activity window) to open it in Finder; Reveal follows a rename→move row to wherever the file ended up. Context menu also copies the path. Selecting a row does not reveal it. Settings → Activity embeds a read-only copy with an **Open Activity** button in the section header and Reveal via double-click and context menu (Control-click); it has no toolbar, to avoid growing the Settings title bar. Each row records the watch folder it came from (rows from before 1.4 count as folder #1); with several folders, rows name their folder and the standalone window's toolbar has a folder filter (All Watch Folders or one folder).

### Cleanup
1. Scan Intake-managed paths (and optionally loose files still in watch root). With several watch folders, Cleanup scans **All Watch Folders** or the one picked at the top of the pane; File Away, Keep and Delete act within the folder the item was found in.
2. Files not opened/modified for *N* days appear in Cleanup — a **decision queue**, never merged with Activity. Multi-select; context menu or toolbar buttons for **File Away**, **Keep**, **Delete…** (Delete key also opens confirmation). Double-click or Space for Quick Look; Reveal in Finder via context menu. Day threshold is shown once — “Unused for [field] days” with a Stepper beside the typeable field; changes rescan after a short pause (~0.5 s), on Return, or when the field loses focus, not on every Stepper click. Each row shows why it's there:
...
3. Actions: **File Away** (pick folder), **Delete** (confirm with item count; Intake moves to Trash), **Keep** (snooze / exclude) — same for every reason. Nothing is ever deleted without the user confirming.
4. After moves/deletes, remove empty Intake-created category folders.

### AI
**Content-aware rename.** Off by default. Text is always extracted on this Mac (PDF text layer first, then Vision for scanned PDFs — first 2 pages — and images; files over 50 MB and encrypted PDFs are skipped; at most 4,000 characters). A **naming provider** then turns that text into fields: **On this Mac** (default) uses Apple's Foundation Models (`SystemLanguageModel`, macOS 26) so file contents never leave the Mac; **OpenRouter** sends the extracted text to your OpenRouter model (requires OpenRouter enabled + Keychain API key) — **contents leave the Mac** when that provider is selected. Both paths return date, document type, organization, subject and a confidence; Intake then renders a deterministic **template** — tokens `{date} {type} {organization} {subject} {original}`, default `{date} {type} {organization}` — dropping empty tokens and dangling separators, accepting only real ISO `YYYY-MM-DD` dates, stripping path separators and control characters, applying the normalizer's clean-up (short all-caps acronyms like `IRS` stay), and capping the name at 80 characters. The result is rejected when empty, only generic words ("Document", a bare date), or under the confidence threshold (0.6); **any rejection keeps the Title Case name**. The extension is always preserved and collision rules match the Title Case rename. Settings (AI pane): **Content-aware rename** section first with master toggle, naming provider picker, file types (PDFs, Images; both by default), template field, **Try on a File…**, and status messages.

**OpenRouter** for folder suggestions: toggle **Suggest folders with AI**, API key, base URL, and model picker. On rule miss / Other, folder suggestions send only a file's name and your rule folder names — never its source, size, or contents. OpenRouter setup (key, URL, model) appears in a disclosure shown only when OpenRouter is enabled. The same OpenRouter account powers content-aware rename when that naming provider is selected. Choosing OpenRouter as the naming provider, or turning on content-aware rename while its provider is already OpenRouter and unacknowledged, prompts a one-time confirmation that extracted text will be sent off this Mac; Cancel leaves content-aware rename off. A persistent status line shows while content-aware rename uses OpenRouter.

**OpenRouter cost tracking**: Intake fetches and displays your API key usage, account limits, and remaining credits whenever OpenRouter is enabled with a valid key. Intake also tracks its own spending in an **Intake spend** section: total across all time and current calendar month, with a reset button. Cost lookups run at most once per minute and only to the configured base URL.

**Error messages**: 401 (invalid key) and 402 (out of credit) get friendly messages in Settings. Non-https base URLs (except localhost) show a warning.
