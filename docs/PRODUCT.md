...
### Activity
Chronological **audit trail** of rename / move / skip / error (plus cleanup delete and empty-folder removal). It is **not** the Dock default. It is not the Cleanup queue. Rows persist across launches. Double-click a row, or use Reveal in Finder (context menu; also a toolbar button in the standalone Activity window) to open it in Finder; Reveal follows a rename→move row to wherever the file ended up. Context menu also copies the path. Selecting a row does not reveal it. Settings → Activity embeds a read-only copy with an **Open Activity** button in the section header and Reveal via double-click and context menu (Control-click); no toolbar to avoid growing the Settings title bar. Each row records the watch folder it came from (rows from before 1.4 count as folder #1); with several folders, rows name their folder and the standalone window's toolbar has a folder filter (All Folders or one folder).
